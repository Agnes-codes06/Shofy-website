...
const GetProducts = ({ searchTerm, selectedCategory, addToCart, toggleWishlist, wishlist }) => {
  ...
  useEffect(() => {
    axios.get("https://agnes.alwaysdata.net/api/getproductdetails").then(res => setProducts(res.data));
  }, []);
  ...
  const filtered = products.filter(item => {
    ...
    // 1. Check for the specific [Tag] first (New System)
    const hasTag = desc.includes(`[${s}]`);

    // 2. Backup: Check for keywords in the name (Old System - Fixes your 4 laptops)
  ...
  return (
    <div className="product-page-container" style={{backgroundColor: '#fcd5d9', minHeight: '100vh', padding: '40px 0'}}>
            ...
            currentItems.map((p, index) => (
              <div className="col-md-3 mb-4" key={index}>
                <div className="card h-100 shadow-sm border-0 product-card">
                  ...
                  <img src={`https://agnes.alwaysdata.net/static/images/${p.product_photo}`} className="product_img p-3" alt={p.product_name} />
                  <div className="card-body text-center d-flex flex-column">
                    <h6 className="fw-bold mb-1">{p.product_name}</h6>
                    {/* Cleans up the tag from the view if it exists */}
                    ...
                    <div className="d-grid gap-2 mt-2">
                      <button className="btn btn-dark btn-sm">Pay Now</button>
             ...
             <div className="text-center py-5"><h4>No products found in {selectedCategory}</h4></div>
          )}
        </div>
